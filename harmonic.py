...
def initialize_filterbank(sample_rate, n_harmonic, semitone_scale, n_filter):
    # MIDI
    # lowest note
...
class HarmonicSTFT(nn.Module):
    def __init__(self,
        ...
        harmonic_hz, self.level = initialize_filterbank(sample_rate, n_harmonic, semitone_scale, n_filter)
        print("init {}".format(harmonic_hz[:self.level]))
    ...
    def get_harmonic_fb(self):
        # bandwidth
        bw = (self.bw_alpha * self.f0 + self.bw_beta) / max(1.0, self.bw_Q)
